...
function V24Acceptance() { const [data, setData] = useState<any>(null); const [error, setError] = useState<string | null>(null); const load = async (path: string) => { try { setError(null); setData(await api<any>(path)); } catch (e) { setError(String(e)); } }; const run = async (scenarioId: string) => { try { setError(null); const accepted = await api<any>("/api/v1/acceptance/v2/v2.4/runs", { method: "POST", body: JSON.stringify({ scenarioId, seed: 20260907 }) }); setData(await api<any>(`/api/v1/acceptance/v2/v2.4/runs/${accepted.testRunId}`)); } catch (e) { setError(String(e)); } }; return <main><header><p className="eyebrow">StockQuant · 开发验收中心</p><h1>V2.4 真实时钟下持续模拟交易</h1><p>PAPER / LIVE_SOURCE_SMOKE / FAKE；仅模拟 Mandate，LIVE 激活关闭。</p></header>{error && <p role="alert" className="error">{error}</p>}<section><h2>运行配置</h2><button onClick={()=>void load("/api/v1/acceptance/v2/v2.4/preview")}>查看采样与执行窗口</button><button onClick={()=>void run("normal")}>运行持续采样</button><button onClick={()=>void run("rejection")}>运行陈旧/重复拒绝</button><button onClick={()=>void run("recovery")}>运行断网恢复</button></section>{data && <pre data-testid="v24-evidence">{JSON.stringify(data,null,2)}</pre>}</main>; }
