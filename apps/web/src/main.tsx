...
function V21Acceptance() { const [data, setData] = useState<any>(null); const [error, setError] = useState<string | null>(null); const load = async (path: string) => { try { setError(null); const r = await api<any>(path); setData((old: any) => ({ ...(old ?? {}), [path.split("/").pop()!]: r })); } catch (e) { setError(String(e)); } }; const over = async () => { try { const securities = Array.from({ length: 101 }, (_, i) => `${String(600000 + i).padStart(6, "0")}.SH`); await api("/api/v1/acceptance/v2/v2.1/watchlist", { method: "PUT", body: JSON.stringify({ securities }) }); } catch (e) { setError(String(e)); } }; return <main><header><p className="eyebrow">StockQuant · 开发验收中心</p><h1>V2.1 免费行情、新闻聚合与在线股票池</h1><p>真实来源能力与小样本验收；不触发交易。</p></header>{error && <p role="alert" className="error">{error}</p>}<section><h2>来源与股票池</h2><button onClick={()=>void load("/api/v1/acceptance/v2/v2.1/sources")}>检查来源能力</button><button onClick={()=>void load("/api/v1/acceptance/v2/v2.1/watchlist")}>查看股票池</button><button onClick={()=>void load("/api/v1/acceptance/v2/v2.1/quotes")}>采集实时快照</button><button onClick={()=>void load("/api/v1/acceptance/v2/v2.1/news/live")}>抓取实时新闻</button><button onClick={()=>void load("/api/v1/acceptance/v2/v2.1/news")}>查看新闻去重样本</button><button onClick={()=>void over()}>提交101只并验证拒绝</button></section>{data && <pre data-testid="v21-evidence">{JSON.stringify(data,null,2)}</pre>}</main>; }
