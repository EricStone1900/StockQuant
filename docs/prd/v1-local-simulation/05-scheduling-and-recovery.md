...
状态：PASS（模拟切片、真实 NATS/JetStream、platform-api 正式进程 NATS 接入、Temporal SDK Worker/Workflow、Worker 重启恢复、Activity 重试和用户人工验收完成，2026-09-09）；Temporal 业务正式进程接入仍未验证。真实 Ubuntu 主机烟测已由用户人工确认，环境明细待补充。版本入口：[README](./README.md)。共同约束：[三版共同规则](../05-three-version-delivery.md)。
