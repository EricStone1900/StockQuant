...
- [ ] 建立BacktestRun及固定Manifest：数据、股票池、策略/因子、规则、成本、依赖/镜像、种子、初始账户、区间与基准。
- [ ] 实现按运行隔离的历史时钟与日线撮合；日终确认信号最早下一可交易时点执行，原价用于成交，复权研究与公司行动入账分离。
- [ ] 增加费用、滑点、成交量约束、停牌/限制价、可卖量及结算处理；日线无法确定的盘中顺序采用明确保守规则。
- [ ] 提供收益/基准、回撤、换手、费用归因、成交率、拒绝/未成交数量和数据覆盖；关联实际模拟流水。
