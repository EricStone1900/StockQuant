...
- [ ] 按版本数据阶梯扩容，先10～20只约60交易日分钟，再按来源覆盖扩大；先测预计磁盘/内存/耗时，不承诺Mac可一次跑全市场多年。
...
- [ ] Mac测50/80/100监控池与小/中历史样本资源；目标Ubuntu架构烟测、备份恢复，记录预定验收预算与实测，归档V2证据。
...
- [x] 第2节已完成小规模扩容切片，真实依赖与 Fixture 替身明确；全量扩容仍未完成。
...
| Fixture文件/数据版本/确切日期/Hash、规则与成本版本 | `v2.5-cn-minute-20x60-v1`；20证券×60交易日×1200行；Manifest见 `docs/evidence-v2.5.md` |
