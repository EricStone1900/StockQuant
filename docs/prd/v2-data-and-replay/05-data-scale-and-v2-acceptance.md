...
- [ ] 按版本数据阶梯扩容，已完成10～20只约60交易日分钟切片；全市场多年数据仍需真实带 Manifest 数据后验证，不承诺 Mac 可一次跑全市场多年。
- [ ] 建立分区查询、增量导入、缓存键、任务排队/取消、并发与存储配额；扩容不改变小样本结果。
- [ ] 补基础模型的训练/验证/测试和Walk-forward、参数敏感度、成本/容量对比；测试集不参与调参，展示失败候选和覆盖不足。
- [ ] 将在线监控100只上限与历史股票池独立配置；真实历史财务/行业PIT不足时相关策略不激活，美股实际来源未接入保持范围标识。
- [ ] Mac 已完成50/80/100数量路径实测，但有效时间戳质量门禁未通过（37/50、67/80、87/100）；目标Ubuntu架构烟测、备份恢复和全市场多年数据仍待真实环境/数据。
