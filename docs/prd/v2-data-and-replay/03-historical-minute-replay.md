...
- [ ] 在量化服务计算Worker中建立完整HistoricalReplayRunner，编排服务只管理启动/取消/检查点。当前独立 Worker 已提供运行查询、暂停、恢复、取消和逐 Bar 取消屏障；完整跨服务 Runner 编排仍待补齐。
- [ ] 历史执行Adapter归交易执行域；当前已接入交易执行服务、治理授权、FakeBroker 和组合账本，并支持显式参与率、买卖方向滑点、限价 DAY 不可成交过期及非法规则拒绝；完整下一可用 Bar/多窗口部分成交和跨服务验收仍待补齐。
...
- [ ] 第2节后端任务全部完成；独立 Worker、跨服务执行/账本闭环和检查点恢复已验证，完整 Runner 编排及历史执行模型仍未完成。
