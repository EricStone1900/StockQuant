...
- [ ] 在量化服务计算Worker中建立HistoricalReplayRunner，编排服务只管理启动/取消/检查点。行情分区在Worker顺序消费，不逐Bar写Temporal历史或广播全市场事件。
- [ ] 历史执行Adapter归交易执行域；复用风控、授权、账户和订单规则，通过运行隔离端口驱动。小样本验收必须经过真实领域服务，快路径若进程内复用领域库须做一致性对照。
...
- [ ] 持久检查点保存行情游标、模拟时间、种子状态、待处理事件和业务版本，恢复不重复成交；报告标MINUTE_BAR，不能宣称逐笔队列复原。
