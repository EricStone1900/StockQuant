...
| [V2.3 日频决策、分钟撮合与历史事件回放](./03-historical-minute-replay.md) | PARTIALLY_IMPLEMENTED（Fixture 回放、持久 TestRun、独立 Worker、多 Bar 检查点、Qlib 训练与独立验证、候选精确版本登记/显式审批、治理授权、费用、基础订单状态转换、成交事务外盒、后台重试扫描、UNKNOWN 安全恢复与 Saga 状态机） | 历史人工记录保留；多 Bar、研究训练/独立验证、候选登记、Web/CLI 与 Saga 验收已完成 | `docs/evidence-v2.3.md`（历史证据已 supersede）；[V2.3-2 技术复核](../../../docs/evidence-v2.3-2.md) | 候选登记暂为进程内存，激活保持 `DISABLED_UNTIL_MANDATE`；尚未接入真实模型训练、完整研究运行时或 LIVE 激活 | 历史用户确认 / 2026-09-09；技术复核 / 2026-09-11 |
