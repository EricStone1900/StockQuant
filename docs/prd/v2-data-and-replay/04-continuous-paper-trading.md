...
- [x] 重复/恢复验证完成；实际观察已开始，V2.4 当前有效日为 `4/20`（DC-08A 20 只集合独立统计为 `1/20`）。
- [x] 用户已通过Web及命令证据完成人工验收，记录确认时间/结论；20 个实际交易日观察仍在累计。
...
手册状态：VERIFIED_EXECUTABLE（确定性场景自动检查及实现范围人工 Web 验收已通过；20 个实际交易日观察进行中，V2.4 当前有效日 4/20；DC-08A 20 只集合独立统计为 1/20）。
