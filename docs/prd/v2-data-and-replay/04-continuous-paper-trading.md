...
- [x] 重复/恢复验证完成；实际观察已开始，V2.4 当前日终有效日为 `6/20`（2026-09-15～18、21～22；DC-08A 20 只集合独立统计为稳定完整日 `2/20`，另有恢复后完整日 `1`）。
...
手册状态：VERIFIED_EXECUTABLE（确定性场景自动检查及实现范围人工 Web 验收已通过；20 个实际交易日观察进行中，V2.4 当前日终有效日 6/20；DC-08A 20 只集合独立统计为稳定完整日 2/20，另有恢复后完整日 1）。
