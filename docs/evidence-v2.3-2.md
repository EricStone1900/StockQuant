...
### 并发压力边界与修复（2026-09-15）

- 初始 10×12 Bar 压力切片触发组合服务连接/请求耗尽，10/10 在 cursor=0 返回 `fetch failed`，该次明确记录为 FAIL；重启组合服务后单任务恢复正常。
- Worker 增加 `STOCKQUANT_REPLAY_MAX_CONCURRENCY`（默认 2）全局回放槽位，额外任务排队，避免同时打满下游连接池。重建后执行 4×4 Bar 回归，4/4 HTTP 200、cursor=4，总耗时约 185ms，组合服务保持 ready。
