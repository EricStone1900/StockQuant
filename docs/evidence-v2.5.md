...
全量预检与受控启动（2026-09-11）：500 档实测推算 5,219 标的约需 538.7 MiB；本机 `/tmp` 可用空间约 183 GiB，磁盘不是阻塞因素。随后启动 `--sample-size 5219`，BaoStock 筛选结果确认 5,219 个已上市 A 股；完成前 20 个证券、29,120 行后按受控策略停止，因单证券请求吞吐预计需要数小时，未将未完成任务标记为 PASS。`/tmp/stockquant-baostock-daily-s5219/manifest.json` 保留 `RUNNING` checkpoint，可在限流窗口或更高吞吐源可用时续跑；当前全市场多年容量门禁仍为 `NOT_RUN`。

全市场多年数据容量验证：`NOT_RUN`。已完成真实免费源的 A 股宇宙筛选和 5/100 标的多年日线分档，尚未执行 5,219 标的全量导入、磁盘/恢复/限频长期验证，也没有目标 Ubuntu 实机；不能用分档结果代替该门禁。
