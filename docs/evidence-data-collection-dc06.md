...
结果：16/16 单元测试通过；真实 PostgreSQL 集成测试 9/9 通过（项目认证、错误令牌拒绝、并发配额）；TypeScript、文档、合同和 Fixture 检查通过。HTTP 烟测验证同项目允许、跨项目返回 403、数据库模式缺失/错误令牌返回 403、分页返回稳定 DataVersion 游标、导出 SHA-256，以及物理去重键生成。

## 未完成门槛

DC-06 当前为 `IN_PROGRESS`。项目规则、持久项目策略/令牌校验、并发配额计数和 API 已完成并验证；公平队列运行指标、分页数据真实 Artifact 存储、导出脱敏和 Web/验收中心 E2E 仍需后续部署交付。无数据库本地模式仍允许开发 Header，正式数据库模式不会接受 Header scopes 伪造。
