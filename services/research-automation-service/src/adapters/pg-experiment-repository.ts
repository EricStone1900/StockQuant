...
type Row = {
  experiment_id: string; fixture_id: string; model_profile: string; rounds: number; budget_cents: number;
...
const mapRow = (row: Row): Experiment => ({
  ...
  rounds: row.rounds, budgetCents: row.budget_cents, environmentMode: row.environment_mode,
...
export class PgExperimentRepository implements ExperimentRepository {
  ...
  async initialize(): Promise<void> {
    await this.pool.query(`CREATE TABLE IF NOT EXISTS research_experiments (
      ...
      rounds INTEGER NOT NULL CHECK (rounds BETWEEN 1 AND 3), budget_cents INTEGER NOT NULL CHECK (budget_cents > 0),
  ...
  async save(experiment: Experiment): Promise<Experiment> {
    const result = await this.pool.query<Row>(`INSERT INTO research_experiments(experiment_id,fixture_id,model_profile,rounds,budget_cents,environment_mode,broker_mode,idempotency_key,status,reason,created_at,cancelled_at)
      ...
      RETURNING *`, [experiment.experimentId, experiment.fixtureId, experiment.modelProfile, experiment.rounds, experiment.budgetCents, experiment.environmentMode, experiment.brokerMode, experiment.idempotencyKey, experiment.status, experiment.reason, experiment.createdAt, experiment.cancelledAt]);
