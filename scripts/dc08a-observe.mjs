...
export async function capture({ subscriptionId = process.env.DC08A_SUBSCRIPTION_ID ?? "dc08a-20260914-short-v1", outputDir = process.env.DC08A_OUTPUT_DIR ?? "evidence/dc08a", readyUrl = process.env.DC08A_MARKET_URL ?? "http://127.0.0.1:3002/ready", now = new Date(), observationCounted = false } = {}) {
  ...
  const pendingOutbox = query(`SELECT count(*) FROM market_data_collection_outbox o JOIN market_data_collection_runs r ON r.run_id=o.run_id WHERE r.subscription_id='${subscriptionId.replaceAll("'", "''")}' AND r.status <> 'CANCELLED' AND o.sent_at IS NULL`).trim() || "0";
